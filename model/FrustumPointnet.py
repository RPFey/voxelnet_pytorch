...
class FrustumPointnet(nn.Module):
    def __init__(self, config, type2class, type_mean_size):
        super(FrustumPointnet, self).__init__()
        self.config = config
        self.config['num_cls'] = self.config['num_size_cluster']
...
class FrustumLight(ModelBase):
    def __init__(self, config, args, type2class, type_mean_size):
        super(FrustumLight, self).__init__(config)
        self.net = FrustumPointnet(config, type2class, type_mean_size)
    ...
    def predict(self, pointcloud, onehot):
        batch_size = pointcloud.shape[0]
        endpoint = self.net(pointcloud, onehot)

        heading_cls = torch.argmax(endpoint['heading_score'], 1)
        size_cls = torch.argmax(endpoint['size_scores'], 1)
        heading_res = endpoint['heading_residuals'][list(range(batch_size)), heading_cls]
        size_res = endpoint['size_residuals'][list(range(batch_size)), size_cls]

        centers = endpoint['center']

        return centers, heading_cls, heading_res, size_cls, size_res   
    ...
    def loss(self, mask_label, center_label, heading_class_label, heading_residual_label,
        ...
        gt_mask = torch.mul(hcls_onehot.unsqueeze(2).repeat(1, 1, self.config['num_size_cluster']),
